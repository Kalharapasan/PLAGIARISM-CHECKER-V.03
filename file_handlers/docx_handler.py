...
class DOCXHandler:
    ...
    def create_sample_docx(self, output_path: str, content: str = None) -> bool:
        try:
            ...
            subtitle = doc.add_heading('Generated by Plagiarism Checker Pro', 1)
            subtitle.alignment = WD_ALIGN_PARAGRAPH.CENTER
            doc.add_paragraph('')
            metadata_para = doc.add_paragraph()
            metadata_para.add_run('Document ID: ').bold = True
            metadata_para.add_run(f'PLG-{datetime.now().strftime("%Y%m%d-%H%M%S")}')
